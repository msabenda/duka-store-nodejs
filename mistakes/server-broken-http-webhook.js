// 🚨 MISTAKE 3C/3D: Webhook URL built with HTTP (not HTTPS)
//
// What's wrong?  The APP_URL replacement from http:// → https:// has been removed.
// If .env sets APP_URL=http://localhost:8002 or APP_URL=http://abc123.ngrok.io,
// the webhook URL will start with http:// — which Snippe rejects.
//
// The error:
...
//   { "status": "error", "code": 400,
//     "error_code": "validation_error",
//     "message": "webhook_url localhost URLs are not allowed" }
//
// Fix: Add back .replace('http://', 'https://') on the APP_URL assignment (line 22)
// ----------------------------------------------------------------------------------
...
// 🚨 BUG: .replace('http://', 'https://') has been removed!
// If APP_URL is http://... the webhook URL will be HTTP → rejected by Snippe
...
async function processPayment(req, res, paymentMethod) {
  ...
  // 🚨 webhookUrl will be http://localhost:8002/webhook (not HTTPS!)
